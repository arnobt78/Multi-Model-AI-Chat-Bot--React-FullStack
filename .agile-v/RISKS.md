...
| RISK-0002 | **High** | Open | Unauthenticated analytics write/read APIs → spam DB, inflate Postgres/Vercel costs, data pollution | REQ-0010 | Rate limits, payload caps, protect dashboard reads |
...
| RISK-0004 | **Medium** | Mitigated | Model IDs refreshed (Groq gpt-oss; OpenRouter `:free`; HF `:fastest`) | REQ-0012 | Registry `models[]` + periodic re-verify |
| RISK-0010 | **Low** | Accepted | HF free Inference Providers (~$0.10/mo) often cannot serve; forced HF shows friendly error | REQ-0012 | Prefer Auto/Groq/Gemini/OpenRouter; keep HF late in fallback |
...
| RISK-0008 | **Medium** | Open | New PrismaClient per serverless invoke in each `api/*.ts` → connection pressure on Postgres | REQ-0010 (related) | Shared singleton / `@prisma/client` pattern for serverless |
